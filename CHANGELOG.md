...
### Fixed
- `encounter.rs` : replace UTC with UTC with timezone in the resale saved link active invalidation

### [2.1.11] - 2026-09-13

### Added
- `.gitignore`: private folder that contains unversioned files like notes
- `CONTRIBUTING.md`: add a section about the project's structure and the crates organization

### Changed
- `.devcontainer` : added a Dockerfile to build the project in a container and provide a development environment compatible with nvim workflow.

### Fixed
- `encounter.rs` : fix the HTML title tag as the only way to identify a match.
...
### Added
- Add proxy support for the parser fetching functionality, allowing the use of a list of proxy servers or a proxy API service to avoid IP bans and improve scraping reliability.
- Provide documentation explaining how to use the admin panel to manage the bot.

## [2.1.8] - 2026-05-25

### Added
- Current application version in the web admin panel.
- Stop/Resume toggle in the admin panel to control scanner runtime state.

### Changed
- Web admin panel icon and browser tab title.
